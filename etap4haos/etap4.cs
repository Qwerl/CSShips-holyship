    ...
    private int thePortSleeptime;
      ...
      { 
        ...
        while(life)
        {
          ...
          {
              Console.WriteLine("Судно №{0} прибыло в порт",N );//информация для отладки
      ...
      public void HandlerEv(Data D)
      {
          ...
          { this.xPort = d.X; this.yPort = d.Y; this.thePortSleeptime = d.Sleeptime; }
      }
   }
...
class Window : Form
{
  ...
  public Window ()
  {
      ...
      port1 = new Port(1, 20, 20, 8000, this);   //зелёный порт
      port1.evPort += new DelShip(this.HandlerEv);
      port2 = new Port(2, 300, 230, 2000, this); //синий порт
      port2.evPort += new DelShip(this.HandlerEv);
      port3 = new Port(1, 50, 300, 1000, this);  //желтый порт
      port3.evPort += new DelShip(this.HandlerEv);
      
      ship = new Ship(104, 100, 100, 20);//создать объект корабля с параметрами : первый параметр номер корабля , второй Х ,третий У , четвертый скорость
