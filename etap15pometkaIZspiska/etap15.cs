...
class Window : Form //окошко
{
    ...
    private int num;                        //количество кораблей
    Port port1, port2, port3;               
    ArrayList ships;                        //массив кораблей
    ...
    {
        num++;
        Ship ship = new Ship(num, 500, 500, randomSpeed.Next(100, 500), Color.FromArgb(randomSpeed.Next(100, 500)/2,0,0));
        port1.evPort += new DelShip(ship.HandlerEv);
        port2.evPort += new DelShip(ship.HandlerEv);
        port3.evPort += new DelShip(ship.HandlerEv);
        ships.Add(ship);
    ...
    private void DelAll(object obj, EventArgs args)
    {
        ...
        else
        {
           ...
           */
            ...
            for (int i = 0; i < ships.Count; i++) //убийца кораблей
            {
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        g.DrawImage(Ships.Properties.Resources.PORTprozrachn_TEN_, new Point(port1.X - 30, port1.Y));
        ...
        */
        ...
        {
            Ship ship = (Ship)ships[i];
            e.Graphics.FillEllipse(new SolidBrush(ship.ShipColor), ship.X, ship.Y, 50, 20);//нарисовать элипс с цветом красным (яркость зависит от скорости) , координатами корабля1 шириной 50 высотой 20
            e.Graphics.DrawString(ship.N.ToString(), aFont, Brushes.Black, ship.X + 18, ship.Y);      //надпись на первый корабль
        }
    }
